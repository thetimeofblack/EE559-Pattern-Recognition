...
def trainNearestClassifierMeanThreeClass(training) :
    SampleMeanClass1 = np.zeros(13)
    SampleMeanClass2 = np.zeros(13)
    SampleMeanClass3 = np.zeros(13)
    CountClass1  = 0
    CountClass2 = 0
    CountClass3 = 0
    for data in training:
        RawData = data[:,range(0,13)]
        if data[13] == 1:
            SampleMeanClass1 += RawData
        elif data[13] ==2 :
            SampleMeanClass2 += RawData
        else :
            SampleMeanClass3+= RawData
    return SampleMeanClass1, SampleMeanClass2,SampleMeanClass3


def trainNearestClassifierOne2Rest(Training , ClassType):
    SampleMeanOne = np.zeros(13)
    SampleMeanRest = np.zeros(13)
    CountClassOne = 0
    CountClassRest = 0
    for data in Training:
        RawData = data[:,range(0,13)]
        if data[13] == ClassType:
            SampleMeanRest += RawData
